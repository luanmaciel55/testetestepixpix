...
module.exports = async function handler(req,res){
  ...
  const logRequest = {
    method:'GET',
    url:`${PAGBANK_API}/orders/${encodeURIComponent(orderId)}`,
  ...
  try{
    ...
    if(!response.ok){
      return res.status(response.status).json({
        error:'Não foi possível consultar o pedido no Sandbox.',
    ...
    return res.status(500).json({error:'Falha interna ao consultar o Sandbox.'});
  }
};
